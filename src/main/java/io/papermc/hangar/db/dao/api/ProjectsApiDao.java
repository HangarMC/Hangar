...
public interface ProjectsApiDao {
    ...
    @DefineNamedBindings
    List<Project> listProjects(String owner, String slug, @Define boolean seeHidden, Long requesterId, String orderBy,
    ...
    @RegisterColumnMapper(PromotedVersionMapper.class)
    @DefineNamedBindings
    List<Project> listProjects(long id, @Define boolean seeHidden, Long requesterId, String orderBy,
                               @BindList(onEmpty = BindList.EmptyHandling.NULL_VALUE) List<Integer> categories,
                               @BindList(onEmpty = BindList.EmptyHandling.NULL_VALUE) List<String> tags, //TODO: implement tags with mc_version('data')
                               String query, @Define String queryStatement, long limit, long offset);
    ...
    List<ProjectMember> projectMembers(String author, String slug, long limit, long offset);
    ...
    @SqlQuery("SELECT CAST(dates.day AS date) dateKey, coalesce(sum(pvd.downloads), 0) AS downloads, coalesce(pv.views, 0) AS views" +
              ...
              " WHERE p.slug = :slug AND p.owner_name = :author AND (pvd IS NULL OR pvd.project_id = p.id)" +
              " GROUP BY pv.views, dates.day")
    Map<String, ProjectStatsDay> projectStats(String author, String slug, LocalDate startDate, LocalDate endDate);
}
