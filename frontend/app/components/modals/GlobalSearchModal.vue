...
const shortcutModifier = ref("Ctrl");
...
onMounted(() => {
  shortcutModifier.value = /Mac|iPhone|iPad|iPod/.test(navigator.userAgent) ? "⌘" : "Ctrl";
});
  ...
  <div>
    ...
    <Teleport to="body">
      ...
      <Transition name="fade">
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-[#000000] bg-opacity-70" @click.self="closeModal">
          <div
            ref="searchContainer"
            class="w-full max-w-3xl h-[600px] mx-4 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 overflow-hidden flex flex-col"
          >
            <!-- Search Input -->
