...
const kratosPool = new Pool({
    connectionString: "postgresql://hangarauth:<pw>@localhost:1234/hangarauth",
...
const res = await kratosPool.query(`
    ...
    WHERE i.traits ->> 'username' = 'MiniDigger'
    ORDER BY username
`);
...
for (let row of res.rows) {
    ...
    const userId = user?.rows?.[0]?.id || 2;
    if (!userId) {
        console.error("Unknown user " + row.username + "?!", user.rows);
        console.error(row);
    } else {
        try {
