  ...
  <div class="flex mb-4">
    ...
    <div class="ml-2">
      <h1 class="text-2xl text-strong">
        {{ user.name }} <a v-if="!user.isOrganization" :href="forumUserUrl(user.name)" :title="i18n.t('author.viewOnForums')"><IconMdiOpenInNew /></a>
      </h1>
    ...
    <div>
      <Card class="flex flex-col space-y-1 items-end">
        ...
        <span v-for="role in user.roles" :key="role.roleId" :style="{ backgroundColor: role.color }" class="rounded p-1 w-max">{{ role.title }}</span>
      </Card>
    </div>
  </div>
  <hr class="mb-4" />
